...
def _load():
    ...
    try:
        with open(config.DATA_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
        # Migrate: add any keys introduced after the file was first created
...
def get_best_day() -> dict:
    """Return {"date": str | None, "kwh": float}."""
    data = _load()
    return {
        "date": data.get("best_day_date"),
        "kwh":  data.get("best_day_kwh", 0.0),
    }
...
def set_best_day(date_str: str, kwh: float):
    """Unconditionally overwrite best_day — caller is responsible for comparison."""
    data = _load()
    data["best_day_kwh"]  = round(kwh, 3)
    data["best_day_date"] = date_str
    _save(data)
    log.info("Best day saved: %s — %.3f kWh", date_str, kwh)

def repair_best_day():
    """
    Scan daily_history and set best_day to the actual maximum.
    Called after backfill to ensure best_day is always accurate.
    """
    data    = _load()
    history = data.get("daily_history", [])
    if not history:
        return
    best = max(history, key=lambda d: d.get("generation_kwh", 0.0))
    data["best_day_kwh"]  = round(best["generation_kwh"], 3)
    data["best_day_date"] = best["date"]
    _save(data)
    log.info("repair_best_day: %s — %.3f kWh", best["date"], best["generation_kwh"])
...
def update_daily(date_str, generation_kwh, export_kwh, import_kwh):
    """
    Upsert one day's record and recompute cumulative totals and best_day.
    """
    data     = _load()
    history  = data["daily_history"]
    ...
    # Update best_day if this day beats the current record
    if generation_kwh > data.get("best_day_kwh", 0.0):
        data["best_day_kwh"]  = round(generation_kwh, 3)
        data["best_day_date"] = date_str
        log.info("New best day: %s — %.3f kWh", date_str, generation_kwh)
